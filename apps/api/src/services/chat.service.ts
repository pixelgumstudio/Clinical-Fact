...
import europePmcService, { EuropePmcResult } from './europePmc.service';
import googleImageSearchService, { GoogleImageResult } from './googleImageSearch.service';
...
${context}`;
  ...
  /**
   * Chat using live medical literature (Europe PMC) and clinical images (Google
   * Custom Search) instead of the local Qdrant document store.
   */
  async chatMedicalLive(
    ...
    const [literatureSettled, imagesSettled] = await Promise.allSettled([
      europePmcService.search(userQuery, 5),
      googleImageSearchService.search(userQuery, 3),
    ]);
    ...
    if (imagesSettled.status === 'rejected') {
      console.error('❌ Google Image search failed:', imagesSettled.reason);
    }

    const context = literatureResults
      .map(r => `[${r.index}] "${r.title}" (${r.year}, ${r.authors})\nAbstract: ${r.abstract}`)
      .join('\n\n---\n\n');

    const systemInstruction = `You are ClinicFact, a medical reference AI. Answer the user's question using ONLY the sources provided below — do not rely on prior knowledge or invent facts. Cite every claim inline using the matching bracketed number (e.g. [1], [2]).
...
${context || 'No sources found.'}`;
    ...
    const text = await geminiService.chat(messages, systemInstruction);

    console.log(`✅ Generated live medical chat response (${text.length} chars) using ${literatureResults.length} sources, ${imageResults.length} images`);
