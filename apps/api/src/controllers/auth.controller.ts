...
export const appleAuth = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!identityToken) {
      res.status(400).json(
        errorResponse('Apple identity token is required', ERROR_CODES.VALIDATION_ERROR)
      );
      return;
    }
    ...
    // Apple only sends email on first sign-in; fall back to client-provided email
    const email = (payload.email as string | undefined) || clientEmail;

    if (!email) {
      res.status(400).json(
        errorResponse('Email not provided by Apple', ERROR_CODES.VALIDATION_ERROR)
      );
      return;
    }
    ...
    res.status(500).json(
      errorResponse('Apple authentication failed', ERROR_CODES.SERVER_ERROR)
    );
  }
};
