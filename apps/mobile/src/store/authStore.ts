...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  signOut: async () => {
    ...
    const cacheKeys = allKeys.filter((k) => k.startsWith('clinicfact-query-cache-'));
    ...
    await Promise.all([
      clearTokens(),
      AsyncStorage.removeItem('@clinicfact:signup_complete'),
    ]);
    set({
      user: null,
      isAuthenticated: false,
  ...
  setOnboardingComplete: () => {
    ...
    AsyncStorage.setItem('@clinicfact:signup_complete', '1').catch(() => {});
  },
    ...
    (async () => {
      try {
        const [accessToken, localCompleteFlag] = await Promise.all([
          getAccessToken(),
          AsyncStorage.getItem('@clinicfact:signup_complete'),
        ]);
