...
export const ChatFileSelectScreen = () => {
  ...
  const attachToSessionId = route.params?.attachToSessionId;
  const isAttachMode = !!attachToSessionId;
  ...
  const [uploadType, setUploadType] = useState<'image' | 'document' | 'audio'>('image');
...
const checkChatLimit = async () => {
  ...
  // "Upload file" from the chat's "+" menu skips this screen's browse UI entirely and opens
  // the native picker immediately, instead of requiring an extra tap on "Upload new document".
  useEffect(() => {
    if (autoUpload && !hasTriggeredAutoUpload.current) {
  ...
  const handleFilePress = async (file: FileItem) => {
    if (isAttachMode && attachToSessionId) {
      try {
        const response = await api.attachSourceToSession(attachToSessionId, { noteId: file.id });
        if (response.success) {
          navigation.goBack();
        } else {
          Alert.alert('Error', response.message || 'Failed to attach note');
        }
      } catch (error: any) {
        Alert.alert('Error', error.message || 'Failed to attach note');
      }
      return;
    }
  ...
  const handleUploadDocument = async () => {
    if (isAttachMode) {
      // Attaching to an existing chat doesn't consume a new-chat quota slot.
      _doUploadDocument();
      return;
    }
    const isAllowed = await checkChatLimit();
  ...
  const _doUploadDocument = async () => {
    try {
      ...
      // wired up to a text extractor yet, so it's deliberately not offered here.
      ...
      if (uploadResponse.success && uploadResponse.data) {
        ...
        if (isAttachMode && attachToSessionId) {
          ...
          const attachResponse = await api.attachSourceToSession(attachToSessionId, { fileId });
          if (attachResponse.success) {
            ...
            throw new Error(attachResponse.message || 'Failed to attach document');
          }
          return;
        }
      ...
      if (uploadResponse.success && uploadResponse.data) {
        ...
        if (isAttachMode && attachToSessionId) {
          ...
          if (attachResponse.success) {
            ...
            throw new Error(attachResponse.message || 'Failed to attach image');
          }
          return;
        }
  ...
  const _doUploadAudio = async () => {
    try {
      ...
      // Transcription happens synchronously server-side (unlike PDF/image which return a
      // fileId first and process separately) — the response already includes the transcript.
      const uploadResponse = await api.uploadAudioWithTranscription({
        uri: file.uri,
      ...
      if (uploadResponse.success && uploadResponse.data) {
        const fileId = uploadResponse.data.fileId;
        console.log('✅ Audio uploaded & transcribed:', fileId);
        ...
        if (isAttachMode && attachToSessionId) {
          ...
          if (attachResponse.success) {
            ...
            throw new Error(attachResponse.message || 'Failed to attach audio');
          }
          return;
        }
  ...
  const getTypeTitle = () => {
    const verb = isAttachMode ? 'Attach' : 'Select';
    switch (type) {
      case 'note':
        return `${verb} a note`;
      case 'image':
        return `${verb} an image`;
      case 'document':
    ...
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptySubtitle}>
        {selectedFolder
          ...
          : `You don't have any ${type === 'document' ? 'PDF documents' : type === 'image' ? 'images' : type === 'audio' ? 'audio notes' : 'notes'} yet.\nCreate some notes first, then come back to chat with them.`
        }
      </Text>
    </View>
  );
            ...
            <View style={styles.uploadSection}>
              ...
              {type === 'audio' && (
                ...
                >
                  ...
                  <View style={styles.uploadTextContainer}>
                    <Text style={styles.uploadTitle}>Upload new audio</Text>
                    <Text style={styles.uploadSubtitle}>Upload an audio file and transcribe it instantly</Text>
                  </View>
                  <ChevronRightIcon size={20} color={colors.text.tertiary} />
                </TouchableOpacity>
              )}
