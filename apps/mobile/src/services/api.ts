...
/** Mirrors apps/api's MedicalSearchFilters shape (docs/medical-chat/TRD.md §5). */
export interface MedicalChatFilters {
  ...
  /** Europe PMC always runs regardless of this — these two are ADDED alongside it. Both
...
export interface MedicalChatGroundingSource {
  uri: string;
  title: string;
}
...
class ApiClient {
 ...
 async uploadImageWithOCR(file: {
  ...
  // --- Device Token / Push Notification Endpoints ---
  ...
  async registerDeviceToken(
    ...
    return this.request('/user/device-tokens', {
      method: 'POST',
      body: JSON.stringify({ token, platform, deviceName }),
    }, true);
  }

  async unregisterDeviceToken(token: string): Promise<ApiResponse<any>> {
    return this.request(`/user/device-tokens/${encodeURIComponent(token)}`, { method: 'DELETE' }, true);
  }
  ...
  async generateQuizFromText(
    ...
    return this.request('/quizzes/generate-from-text', {
      method: 'POST',
      body: JSON.stringify({ content, title, questionCount, difficulty, questionTypes, targetLanguage, chatSessionId }),
    }, true, 90000);
  }
  ...
  /** One row per source note/chat (aggregated last/best score + attempt count) instead of
   *  one row per generated quiz — powers the top-level Quiz History list. */
  async getQuizGroups(): Promise<ApiResponse<any>> {
    return this.request('/quizzes/groups', { method: 'GET' }, true);
  }

  /** Every quiz attempt generated under a single note or chat session. */
  async getQuizGroupDetail(params: { noteId?: string; chatSessionId?: string }): Promise<ApiResponse<any>> {
    const query = params.noteId
      ? `noteId=${params.noteId}`
      : `chatSessionId=${params.chatSessionId}`;
    return this.request(`/quizzes/group?${query}`, { method: 'GET' }, true);
  }
  ...
  async generateFlashcardsFromText(
    ...
    return this.request('/flashcards/generate-from-text', {
      method: 'POST',
      body: JSON.stringify({ content, title, cardCount, difficulty, focusTopics, targetLanguage, chatSessionId }),
    }, true, 90000);
  }
  ...
  /** Flashcard sets generated from the same note/chat as a quiz — powers the
   *  "Flashcard history" tab on QuizGroupDetailScreen. */
  async getFlashcardSetsBySource(params: { noteId?: string; chatSessionId?: string }): Promise<ApiResponse<any>> {
    const query = params.noteId
      ? `noteId=${params.noteId}`
      : `chatSessionId=${params.chatSessionId}`;
    return this.request(`/flashcards?${query}`, { method: 'GET' }, true);
  ...
  /** One row per source note/chat (aggregated progress + set count) instead of one row per
   *  generated set — powers the top-level Flashcard History list. */
  async getFlashcardGroups(): Promise<ApiResponse<any>> {
    return this.request('/flashcards/groups', { method: 'GET' }, true);
  }

  /** Every flashcard set generated under a single note or chat session. */
  async getFlashcardGroupDetail(params: { noteId?: string; chatSessionId?: string }): Promise<ApiResponse<any>> {
    const query = params.noteId
      ? `noteId=${params.noteId}`
      : `chatSessionId=${params.chatSessionId}`;
    return this.request(`/flashcards/group?${query}`, { method: 'GET' }, true);
  }
  ...
  /** Standalone "ask anything" medical Q&A session — no note or document required. */
  async createMedicalChatSession(): Promise<ApiResponse<any>> {
  ...
  /** Attach an additional note or file to an existing chat session (mid-conversation "+"). */
  async attachSourceToSession(sessionId: string, source: { noteId?: string; fileId?: string }): Promise<ApiResponse<any>> {
    return this.request(`/chat/${sessionId}/attach`, {
      method: 'POST',
  ...
  /** Builds a URL for a previously-uploaded file's raw bytes (e.g. a chat message's attached
   *  image), served through the API itself rather than a MinIO URL — MinIO's own port isn't
   *  guaranteed reachable from the device (firewall/Docker networking), whereas this app already
   *  depends on the API's port being reachable to work at all. Auth is passed as a query param
   *  (the `authenticate` middleware supports this) since <Image> can't easily attach a custom
   *  Authorization header per-platform. */
  async getFileContentUrl(fileId: string): Promise<string> {
  ...
  async togglePinChatSession(sessionId: string): Promise<ApiResponse<any>> {
    return this.request(`/chat/${sessionId}/pin`, { method: 'PUT' }, true);
  }
