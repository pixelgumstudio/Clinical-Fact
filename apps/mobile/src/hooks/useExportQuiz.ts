...
export const useExportQuiz = () => {
  ...
  const exportQuiz = async (
    ...
    try {
      ...
      const arrayBuffer = await api.getQuizExport(quizId, endpoint, format);
      ...
      // Show success alert
      Alert.alert('Success', 'Quiz exported successfully!');
      ...
      if (openShareDialog) {
        try {
          await Sharing.shareAsync(fileUri, {
            ...
            dialogTitle: `${quizTitle} - ${format.toUpperCase()}`,
          });
        } catch (shareError) {
      ...
      let errorMessage = 'Failed to export quiz.';
  ...
  return { exportQuiz, isLoading, error };
};
