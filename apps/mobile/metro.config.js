...
// 2b. Handle workspace package resolution for @clinicalfact/*
const extraNodeModules = {};
const packages = ['design-system', 'types', 'shared', 'validation'];
packages.forEach(pkg => {
  extraNodeModules[`@clinicalfact/${pkg}`] = path.resolve(workspaceRoot, `packages/${pkg}`);
});
...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  ...
  // Handle @clinicalfact workspace packages - resolve from source
  if (moduleName.startsWith('@clinicalfact/')) {
    const packageName = moduleName.replace('@clinicalfact/', '');
